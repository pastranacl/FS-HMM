...
int main()
{
  ...
  double *x = (double *)malloc( N_ts * sizeof(double *) ),
         ...
         *F = (double *)malloc( N_ts * sizeof(double *) ),
  ...
  // Here the force-extension curve is generated
  F[0] = MAX_FORCE; 
  for (int i = 1; i<N_ts; i++)
    F[i] = F[i-1] - (MAX_FORCE-MIN_FORCE)/N_ts;
  
  
  //
  double eq_z = extension(F[0], Lc[state], Lp[state]),
	 eq_x = 0,
	 eq_y = 0;
  ...
  double k_x = F[0]/eq_z,
	 k_y = F[0]/(eq_z + R),
	 k_z = stiffness_z(eq_z, Lc[state], Lp[state]);
  ...
  {    
      ...
      {
	state = state_trial;
	eq_z = extension(F[i], Lc[state], Lp[state]);
	k_z = stiffness_z(eq_z, Lc[state], Lp[state]);
	k_x = F[i]/eq_z,
	k_y = F[i]/(eq_z + R);
      }
  ...
  fprintf(open_file, "%f\t%d\t%f\t%f\t%f\t%f\n", t[0],states[0],F[0],x[0],y[0],z[0]);
...
{
  return Lc*( 1 - 0.5*sqrt( 1/((p*Force)/kBT -1/32) )   );
}
